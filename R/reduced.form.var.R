"reduced.form.var" <- function(Y, p, z = NULL) {
  ...
  pfit <- list(
    capT = capT, ncoef = ncoef, num.exog = num.exog,
    Sh1 = Sh1
  )
...
"summary.VAR" <- function(object, ...) {
  labels <- c(
    "Reduced Form VAR", "Constants",
    "Posterior Regression Coefficients",
    "Posterior error covariance"
  )
  rf.list <- list(
    labels = c("Number of observations", "Degrees of freedom per equation"),
  ...
  ar.list <- list(
    labels = c("Autoregressive Matrices"),
    values = round(object$ar, 6)
  )
  if (is.na(object$exog[1]) == FALSE) {
    exog.list <- list(
      labels = c("Exogenous variable posterior coefficients"),
      values = round(object$exog, 6)
    )
  } else {
    exog.list <- list(labels = NULL, values = NULL)
  }
  vcv.list <- list(
    labels = c("Posterior error covariance"),
    values = object$mean.S
  )
  values <- list(
    rf = rf.list, ar = ar.list, constants = constants.list,
